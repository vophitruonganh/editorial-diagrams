...
test('layoutGraph positions nodes top-to-bottom with increasing y', async () => {
  const g = await layoutGraph(nodes, edges, { direction: 'TB' });
...
test('layoutGraph returns edge waypoints', async () => {
  const g = await layoutGraph(nodes, edges);
  assert.equal(g.edges.length, 2);
  for (const e of g.edges) assert.ok(Array.isArray(e.points) && e.points.length >= 2);
});

test('layoutGraph honors sizeOf for node geometry', async () => {
  const g = await layoutGraph(nodes, edges, { sizeOf: n => ({ w: 300, h: 120 }) });
