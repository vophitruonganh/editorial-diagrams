...
test('matrix renders a table with headers + row labels', () => {
  const h = renderByType(matrix, css);
  assert.match(h, /<table/);
  assert.match(h, />Enterprise</);
  assert.match(h, />SSO</);
});

test('quadrant renders axes + item dots', () => {
  const h = renderByType(quad, css);
  assert.match(h, />MFA</);
  assert.match(h, /border-radius:50%/);
  assert.match(h, /quick wins/);
});

test('kanban renders columns + cards', () => {
  const h = renderByType(kanban, css);
  assert.match(h, /In progress/);
  assert.match(h, /class="card sec"/);
});

test('swimlane renders lanes + step cards + arrows', () => {
  const h = renderByType(swim, css);
  assert.match(h, />Payment</);
  assert.match(h, /url\(#arrow\)/);
  assert.match(h, /<h4>Charge card<\/h4>/);
});

test('grid validation catches missing arrays + bad swimlane lane', () => {
