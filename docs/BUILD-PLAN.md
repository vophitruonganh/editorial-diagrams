...
## Build status — COMPLETE: full ≥70% scope shipped (2026-06-20)
All phases built inline, TDD, **78 tests pass**, **33 diagram types** across 5 engines (flow · graph · lane · sequence · grid), PNG/PDF/SVG verified. 6 MCP tools. Every notation-heavy type visually verified through the production path.
- **C4 complete:** L1 (System Context) · L2 (Container) · L3 (Component) · L4 (Code = class engine).
- **Done incl. communication** (sequence variant, graph engine). Edge labels render on a top layer (never hidden).
- **Excluded (below threshold):** Venn (~65%), Sankey (~50%); data charts / mockups (different domain).
- **Optional follow-ups:** node auto-measuring for tighter graph layouts; `alt`/`opt`/`loop` frames for sequence; expand `@stores`/`@core` partials.
